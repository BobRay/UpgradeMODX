...
;( function( window ) {
	...
	ProgressButton.prototype._setProgress = function( val ) {
		var ugm_button_text = document.getElementsByClassName('content')[0];
		// console.log('VAL: ' + val);
		...
		if (val <=.1) {
			ugm_button_text.innerHTML = "Submit";
		} else if (val > .1 && val < .6) {
			ugm_button_text.innerHTML = "Downloading Files";
		} else if (val >= .6 && val < .7) {
			ugm_button_text.innerHTML = "Copying Files";
		} else if (val >= .4) {
			ugm_button_text.innerHTML = "Launching Setup";
			val = 1;
		}
